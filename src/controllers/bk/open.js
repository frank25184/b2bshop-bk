...
let moment = require('moment'),
     ...
     Category = require('../models/OpenSourceCategory'),
     SubCategory = require('../models/OpenSourceSubcategory'),
    User = require('../models/User'),
    // Comment = require('../models/ ProductComment'),
    openProxy = require('../db_proxy/opensource'),
...
module.exports = {
    ...
    opensources: async (req,res) => {
        ...
        let opensources = await OpenSource.find({}).exec();

        let query = {hidden: false};
        let sort = {created_at: -1};//时间顺序排，并还可以提供getTen数据唯一性
        let {products, count, isLastPage, error} = await openProxy.getTenAsync(query,sort,page);

        if(p){
            let data = {products, count, isLastPage, error};
            res.json(data); 
        }else{
            res.render('open/openList', {
                opensources,
                 seo: {
                     title: `Open Source Alternatives to Proprietary Software for Indie Hackers`,
                     keywords: `Open Source Alternatives`,
                     description: `Explore open source alternatives to proprietary software for indie hackers to build your products efficiently with greater customization and cost savings.`
                 },
                 env:{
    ...
    makeOpen: async (req,res) => {
       
         res.render('form/open', {
              seo: {
                  title: `Sumbit an Open Source Project`,
                  keywords: `submit Open Source Project`,
                  description: `Submit an Open Source Project and make your project public to the world! `,
              },
              env:{
                  cspNonce: res.locals.cspNonce
              },
    ...
    postSiteForMultiIMG: async (req,res)=>{
        const user = req.user,
        product = new OpenSource();
  
        // Productimage
        let uploadDir = config.uploadDir + 'opensources/thumbnail/'; 
        ...
        form.parse(req,async (err,fields,files)=>{
           ...
           let title = util.trim(xss(fields.title ? fields.title[0] : '')),
            url = util.trim(xss(fields.url ? fields.url[0] : '')),
            ...
            pros =  (fields["pros[]"] || []).map(pro => util.trim(xss(pro))).filter(pro => pro.length > 0),
            cons =  (fields["cons[]"] || []).map(con => util.trim(xss(con))).filter(con => con.length > 0),
            features =  (fields["features[]"] || []).map(feature => util.trim(xss(feature))).filter(feature => feature.length > 0),
            alternatives =  (fields["alternatives[]"] || []).map(alternative => util.trim(xss(alternative))).filter(alternative => alternative.length > 0),
            languages =  (fields["languages[]"] || []).map(language => util.trim(xss(language))).filter(language => language.length > 0),
            license = util.trim(xss(fields.license ? fields.license[0] : '')),
           ...
           // product.alternatives = alternatives;
            ...
            product.languages=languages; //programming langauge
            ...
            if (tertiaryPath && tertiaryPath.length) { 
             ...
             product.save(async (err,product)=>{
                  if(err){
                    ...
                    for (const altName of alternatives) {// 遍历替代应用数组alternativeTo
                        let name_changed = util.urlBeautify(altName);
                        let alternative = await AlternativeTo.findOne({ name_changed });
                        if (!alternative) {    // 如果不存在，则创建新的替代应用
                             let thealternative = new AlternativeTo({ name: altName,name_changed,author:user._id,user_id:user._id, opens:[product._id]});
                             alternative = await thealternative.save();
                        }else{
                            await AlternativeTo.findOneAndUpdate({ _id: alternative._id }, {$push: {opens: product._id}}, {new: true }).exec();// 更新原有数据库
                        }
                        await OpenSource.findOneAndUpdate({ _id: product._id }, {$push: {alternativeTos: alternative._id}}, {new: true }).exec();// 更新原有数据库
                       // let openExist = await AlternativeTo.findOne({opens: product._id}).exec();
                    }
                        ...
                        for (let i = 0; i < categories.length; i++) {
                            ...
                            await openProxy.saveCategory(req, res, cat, Category, OpenSource, product_id, 'category');
                            await openProxy.saveCategory(req, res, cat, SubCategory,OpenSource,product_id,'subcategory');
                        }
                    ...
                    //     res.render('email/sorry',
                    //         {layout:null, user}, (err,html)=>{
                    //             if(err){logger.info('err in email template', err);}
                    //             try{
                    //                 mailService.sendToGroup(emailArray, "Apologies for the Mix-Up and Exciting Updates", html);
                    ...
                    //    );
                        ...
                        res.redirect(`/open/` + product.tertiaryPath);
                  }
            });
    ...
    showSite: async (req,res) => {
       ...
       // logger.info(`ip: ${ip}`);
        openProxy.getSiteByName(req,res,tertiaryPath,'open/showOne'); 
    },
