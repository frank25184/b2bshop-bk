...
// ObjectId
// Array
var promptSubcategorySchema = new Schema({
...
promptSubcategorySchema.methods.time = time=> {
    return moment(time).format('MMMM D, YYYY');
};

promptSubcategorySchema.methods.processPrompt = ca=>{
    let intro = ca.intro;
    if(intro){
...
module.exports = mongoose.model('PromptSubcategory', promptSubcategorySchema);