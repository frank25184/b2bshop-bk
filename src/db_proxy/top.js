...
const Product = require('../models/Product'),
  ...
  TopCategory = require('../models/TopCategory'),
...
module.exports = {
  ...
  modifySite: function (post, cb) {
    let modifiedTop = post.processTop(post)
    cb(modifiedTop)
...
// });
  ...
  getTenAsync: async function (query,sort, page, ...args){
    ...
    try {
      ...
      // 获取文章总数
      const count = await Top.countDocuments(query).exec();

      // 获取指定页的文章列表
      const products = await Top.find(query)
      ...
      const newTop = await this.modifySitesAsync(products);
  ...
  getTen: function (query,sort, page, callback, ...args) {  
    ...
    * @param {String} name
    */
    // getTopByName: async function (req,res,name, path) {
    //         const that = this;
    ...
    //                 return new Promise(function(resolve,reject){
    //                     Top.findOne({'name_changed': theName},function(err,site){
    //                             if (err) {
    //                                 reject(err);
    //                             } else {
    //                                 //setting view times
    ...
    //                                 Top.findOneAndUpdate(conditions, update, function(err,site){
    //                                     if(err){
    ...
    //               let top = await findSite(name);
    //               if(top && top.name){
    //                 let newTop = top.processTop(top);
    //                 logger.info(`new top: ${JSON.stringify(newTop)}`);

    //                 // let tagsArr =  newTop.tagsString.split(',');                      

    //                 let isUpvoted = util.userUpvote(loginedUser,newTop._id);//true or false;

    //               if(!proCat){
    //                 res.redirect('/');
    //               }else{
    //                 logger.info(`IP Address: ${req.ipInfo.ip} is visiting ${name}..`);
    //                 //let alternatives = toptag.posts;
    //                 let alternatives = await Top.find({category: newTop.category}).limit(3).exec();
    ...
    //                     seo: {
    //                       title: `${newTop.name}`,
    //                       keywords: `${newTop.categoryName},${newTop.subcatNamecategoryName}`,
    ...
    //                     product: newTop,
    //                     // tagsArr,
    //                   //  tag: util.formatArray(tagsArr)[0],
    ...
    saveCategory: async function(req,res,name,Model,ProductModel,product_id,kind){
      try{
        ...
        if(kind == 'category'){ //{catName, subcatName};
          logger.info(`into kind ==category`);
          ...
          if (acat) {  //already exist
            ...
            logger.debug('category do not exist...'); 
            category.name_changed = util.urlBeautify(catName);
            let cate = await category.save();
            console.log(`saved category: ${JSON.stringify(cate)}`);
          ...
          logger.info(`into kind == subcategory`);
          ...
          logger.info(`into news category...`);
          let catName = name.catName;
          category.name = catName;
          category.name_changed = util.urlBeautify(catName);
          let acat = await Model.findOne({ name: catName }).exec();
          if (acat) {  //already exist
            ...
            let cate = await category.save();
            await Model.findOneAndUpdate({ '_id': cate._id }, {$inc: { 'count': 1 } }, { new: true }).exec();await ProductModel.findOneAndUpdate({"_id": product_id }, {category: cate._id}).exec();
          }
        }
      }catch (err){
