...
const Subcategory = require('../models/PromptSubcategory');
...
module.exports = {
  ...
  modifySite: function (post, cb) {
    let modifiedPost = post.processPrompt(post)
    cb(modifiedPost)
  },
        /**
...
// });
  ...
  getTenAsync: async function (query,sort, page, ...args){
    ...
    try {
      ...
      // 获取指定页的文章列表
      const products = await Prompt.find(query)
  ...
  getTen: function (query,sort, page, callback, ...args) {  
    ...
    const getCount = new Promise(function (resolve, reject) {
      ...
      Prompt.find().sort(sort).count(query, (err, count) => {
    ...
    getCount.then(function (count) {
      ...
      Prompt.find(query).skip((page - 1) * topicCount).limit(topicCount).sort(sort).exec((err, posts) => {
          if (err) {
    ...
    getSiteByName: async function (req,res,name, path) {
            ...
            if(!name){
                ...
                let findSite =  function (theName){
                    return new Promise(function(resolve,reject){
                        Prompt.findOne({'tertiaryPath': theName},function(err,site){
                                if (err) {
                                    reject(err);
                                } else {
                                    //setting view times
                                    ...
                                    Prompt.findOneAndUpdate(conditions, update, function(err,site){
                                        if(err){
                  ...
                  if(site && site.title){
                    let newSite = site.processPrompt(site);
                    logger.info(`new Prompt: ${JSON.stringify(newSite)}`);

                    // let tagsArr =  newSite.tagsString.split(',');                      

                    let isUpvoted = util.userUpvote(loginedUser,newSite._id,'prompt');//true or false;              
                ...
                //  let categoriesObj = [proCat.name,proSubcat.name];
                  if(!proCat){
                    ...
                    let alternatives = await Prompt.find({subcategories: { $in: newSite.subcategories }}).limit(3).exec();
                    ...
                    alternatives = alternatives.map(function(v){
                      ...
                      res.render(path, {
                        seo: {
                          title: `${newSite.title} - Details,Features & Alternatives 2025`,
                          keywords: `${newSite.title} features,${newSite.title} alternatives`,
